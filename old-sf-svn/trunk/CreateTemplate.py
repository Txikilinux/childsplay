...
"""
  ...
  This simple script to create a activity template and to setup various stuff
...
The string 'act_' will be added in front of the given name.\n\
"
...
correct directory.\n\
If you want to use the default image hit enter.\n\
...
ans = raw_input("\nGive the image path or just 'enter' to use the default :")
if ans:
    check_for_q(ans)
    if not ans:
    ...
    # assuming the default path is used
    ans = "template.icon.png"
...
The rows that will be added automaticaly to your activity dbase table are:\n\
    table_id, user_id, timespend, start_time, end_time, level, score, wrong \n\
    \n\
...
to the SQLTables module with the rows mentioned earlier.\n\
"
...
#-------------- Generating template
import rawtemplate
# add a bogus value to prevent errors from to little values in the template dict
...
os.mkdir('lib/CRData/%s' % data_dict['datadirname'])
...
if not data_dict['iconpath']:
    ...
    icondest = os.path.join('lib/SPData/menu/default','%s.icon.png' % template_dict['name'])
    ...
    it is set to '1,1', meaning first row first column.\n\
    If you don't alter the 'position' attribute your menu will be scrambled\n\
    To edit the node start your editor and open 'lib/SPData/menu/default/SP_menu.xml'\n\
    look at the bottom for an entry like this; <menuitem file=%s.icon.png>.\n\
    Below that there's an entry; <position>1,1</position>\n\
    Change it to the correct position" % template_dict['name']
    print "\nAdding node to SP_menu.xml..."
    tree = ET.parse('lib/SPData/menu/default/SP_menu.xml')
    root = tree.getroot()
    subnode = root.find('submenu')
    node = ET.SubElement(subnode,'menuitem')
    ...
    node1.text = '1,1'
    node2 = ET.SubElement(node,'tooltip')
    node2.text = template_dict['name']
    indent(tree.getroot())
    #tree.write(sys.stdout)
    #sys.stdout.flush()
    print "I will backup the original SP_menu.xml before making alterations..."
    shutil.copyfile('lib/SPData/menu/default/SP_menu.xml','lib/SPData/menu/default/SP_menu.xml.back')
    f = open('lib/SPData/menu/default/SP_menu.xml','w')
