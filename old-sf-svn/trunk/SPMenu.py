...
class Activity:
    ...
    def __init__(self,SPGoodies):
        ...
        self.activityimage = None
        MENUPATH = os.path.join(ACTIVITYDATADIR,'SPData','menu',self.SPGoodies._theme)
        mm = MainMenu(MENUPATH,self.SPGoodies)
        self.icon_hash = mm.xm.get_icon_hash()
        ...
        #self.mainbox.connect('expose_event', self.expose)

        #img = os.path.join(MENUPATH, 'mainmenu_backgr.png')
        #self.menu_pixbuf = gtk.gdk.pixbuf_new_from_file(img)
    ...
    def _detach(self):
        ...
        This is needed so that the core can let activities attach themselfs to
    ...
    def _get_activity_icon_image(self):
        """Called by the core to get the dictionary with the modules and iconpaths.
        Used to set the icon in the bottom menu bar."""
    ...
    def menu_callback(self,widget,table):
        ...
        if type(table) in (types.UnicodeType,types.StringType):
            ...
            self.activityimage = box.get_children()[0]
            # TODO: image doesn't show XXX
            # test with a image from file works ok ???
...
class ParseXMLMenu:
    def __init__(self,MENUPATH,xmlpath):
        ...
        self.logger.debug("Start parsing menu XML: %s" % self.menuxmlpath)
        try:
    ...
    def get_icon_hash(self):
        """Returns a dictionary with the module names and iconpaths.
        This is used by the core to display the correct image for the
        activity that's loaded."""
        return self.icon_hash
    ...
    def _make_submenu(self):
        ...
        for SubMenuNode in self.xmldoc.getElementsByTagName(u'submenu'):
            ...
            # print for debugging
            #print "iconpath", iconpath, "pos:", x, y
        return menu
    ...
    def _make_menuitem(self,nodeslist):
        ...
        for ItemNode in nodeslist:
            ...
            # print for debugging
            #print "file,iconpath",file, iconpath, "pos:", x, y
            menu[file] = (iconpath,(x,y),tooltip)
            self.icon_hash[file] = iconpath
        return menu
    
class MainMenu:
    def __init__(self,MENUPATH,spg):
        ...
        self.xm = ParseXMLMenu(MENUPATH,spg._menupath)
...
#        # Used by the expose callback
#        window = spg.get_parent()
#        style = window.get_style()
#        gc = style.black_gc
#        self.gdk_pixmap, self.mask = gtk.gdk.pixmap_create_from_xpm(window.get_window(),\
#                                                    style.bg[gtk.STATE_NORMAL],\
#                                os.path.join(MENUPATH, 'mainmenu_backgr.xpm'))

#    def expose(self, widget, ev):
#        widget.window.set_back_pixmap(self.gdk_pixmap, False)
#        #widget.window.shape_combine_mask(self.mask, 0, 0)
#
...
#        return True    
    ...
    def get_menuitems(self,cbf,parentbox):
        ...
        if self.xm.which_menu() == 'normal':# normal is single level
            ...
            ocp_menu = {}
            # TODO: find out which dict keys in menu are available
            # try to get the current menu item loaded from disk 
