...
# This holds the from gtkbuilder generated gui
import sp_cr
# misc gtk widgets
import SPWidgets
...
def cleanup():
    ...
    if sys.platform == "linux2":
        module_logger.info("Removing pyc files")
...
class MainCoreGui:
    def __init__(self,parent,options=None,language=('en',False),observer=None):
        ...
        # ensure it is rendered immediately so that the user knows were doing something
        ...
        self.currentactivity_align = self.GUI.frame_menu_icon
    ...
    def set_act_icon_bottom_menu(self):
        act = self.activity.get_name()
        if act == 'menu':
            self.currentactivity_align.remove(self.currentactivity_align.get_child())
            pass
        else:
            # get the icon from the menu and set it as an image in the bottom bar.
            dict = self.menu_saved._get_activity_icon_image()
            img = gtk.Image()
            img.set_from_file(dict[act])
            self.currentactivity_align.add(img)
        self.currentactivity_align.show_all()
    ...
    def core_dice_button_pressed(self,count):
        ...
        if count > self.activity.get_levelscount():
            txt = _("This activity has only %s levels." % self.activity.get_levelscount())
            SPWidgets.InfoDialog(txt)
            #self.activity_game_end()
            return self.activity.get_levelscount()
        else:
            self.activity_level_end()
        return count
    ...
    def activity_game_end(self,store_db=None):
        ...
        try:
            # In case the activity has started secondary threads it must stop them
            self.activity.stop()
        except:
            pass
        dlg = SPWidgets.InfoDialog(_("This activity is finished"))
        dlg.run()
        dlg.destroy()
        ...
        self.set_act_icon_bottom_menu()
        self.start_main_loop()
    def set_dicebutton_active(self, enable):
        """Used by SPGoodies to enable or disable the dice button."""
        self.logger.debug("set_dicebutton_active called with %s" % enable)
        self.dicebut.set_sensitive(enable)
    ...
    def start_activity_loop(self):
        # start stepping through the activity levels
        try:
    ...
    def start_next_level(self,store_db=None):
        ...
        self.logger.debug("start_next_level called")
