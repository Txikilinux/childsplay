...
# check to see if we have sound support.
# if not we set a flag which sound based activities should check.
# XXXXX we don't use sound yet XXXXXXX
NOSOUND = True
#try:
#    import pygame
#    from pygame.constants import *
#except ImportError:
#    module_logger.info("No pygame module found, sound based activities will be disabled")
#    NOSOUND = True
...
def validate_entry_cells_integers(e_list):
        ...
        if not correct:
            ...
            return 0
def reset_entry_cells(e_list):
    for e in e_list:
        e.modify_base(gtk.STATE_NORMAL, gtk.gdk.color_parse(WHITE))
def set_widget_to_red(widget):
...
def xpm_label_box(img_filename, label_text=''):
    ...
    image = gtk.Image()
    #module_logger.debug("trying to set image from file: %s" % img_filename)
