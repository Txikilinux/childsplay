...
def get_data_files():
    ...
    datafiles.append(('./childsplay_sp/ocempgui/widgets/themes/default', [os.path.join(DEFAULTDATADIR, 'themes', 'default', 'default.rc')]))
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == 'py2exe':
        ...
        extra_data = {
            "windows": [{ "script": "./bin/cognitionplay",
        ...
        fpin = fpin.replace('$AppVerName$', 'Cognitionplay ' + SPVersion.version + PATCH_VERSION)
        fpin = fpin.replace('$AppVersion$', SPVersion.version)
        fpin = fpin.replace('$VersionInfoVersion$', SPVersion.version)
        fpin = fpin.replace('$OutputBaseFilename$', 'Cognitionplay-' + SPVersion.version + PATCH_VERSION + '_win32')
...
# **************************************************************************        
        ...
        extra_data = {
            "app": ['/bin/cognitionplay'],
            "setup_requires": ['py2app'],
            "options": {"argv_emulation": True,
                        "iconfile": os.path.join(os.path.dirname(os.path.abspath (sys.argv[0])), "data","Cognitionplay.icns")},
        }
