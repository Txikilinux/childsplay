...
import SQLTables
from SPConstants import DBASE, HOMEDIR
import utils

# get a datamanager
def get_datamanager(theme):
    ...
    Typical usage would be:
        ...
        """
    return DataManagerWrapper(SPGoodiesWrapper(theme))
...
class DataManagerWrapper(SPDataManager.DataManager):
    ...
    def __init__(self,spgoodies):
        ...
        self.spg = spgoodies
        self.dbasepath = os.path.join(HOMEDIR, self.spg.get_theme(), DBASE)
        self.cmd_options = self.spg._cmd_options
        atexit.register(self._cleanup)
        self.logger.debug("Starting dbase, %s" % self.dbasepath)
        self.engine = sqla.create_engine('sqlite:///%s' % self.dbasepath)
