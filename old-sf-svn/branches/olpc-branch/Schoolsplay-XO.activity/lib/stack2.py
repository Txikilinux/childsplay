...
class StackWidget(SimpleGladeApp):
    ...
    def __init__(self, parent=None,numbersrange=100,level=1,\
                glade_path="stack1.glade", root="window_stack10", domain=None):
        self.logger = logging.getLogger("schoolsplay.stack1.MainWindow")
        self.logger.debug("Building a 10 stack widget")
        ...
        self.window_stack10.remove(self.frame1)
        ...
        for i in range(start,end):
            # Our entries are named: entry1, entr2, ..., entry10
    ...
    def _create_exercise(self):
        ...
        for j in range(maxrows):
            self.matrix[j] = [0,]*i
            i -= 1
        # now we put random numbers 0-15 in the first row.
        ...
        for row in range(maxrows-1):# last row is just a solution
            ...
            for f in range(fields-1):
                ...
                i += 1
        print self.matrix
        # determine in which three fields we show the numbers.
        rlist = range(1,11)
        elist = []
        ...
        i = 1# index start for the entries 
    ...
    def _solutionbut_observer(self):
        ...
        if self.stackwidget.check_solution():
            ...
            if self.stackcount < self.total_exercises:
                ...
                self.SPG.tellcore_level_end(store_db)
        else:
