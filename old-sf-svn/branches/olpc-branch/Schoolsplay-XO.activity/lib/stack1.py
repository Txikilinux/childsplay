...
class StackWidget(SimpleGladeApp):
    ...
    def __init__(self, parent=None,numbersrange=20,level=1,\
        ...
        for i in range(start,end):
            # Our entries are named: entry11, entr12, ..., entry15
    ...
    def _create_exercise(self):
        ...
        # first create a matrix and a few attributes to keep track of things.
        self.matrix = {}
        self.wrongsolutions = 0
        ...
        for j in range(maxrows):
            self.matrix[j] = [0,]*i
            i -= 1
        # now we put random numbers 0-15 + level*5 in the first row.
        ...
        for row in range(maxrows-1):# last row is just a solution
            ...
            for f in range(fields-1):
                ...
                i += 1
        #print self.matrix
        # determine in which two fields we show the numbers.
        rlist = range(11,17)
        elist = []
        ...
        i = 11# index start for the entries 
  ...
  """
    ...
    def _set_stackwidget(self):
        ...
        self.stackwidget = StackWidget(parent=self,\
                            ...
                            glade_path=os.path.join(self.my_datadir,'stack1.glade'))
    ...
    def _numbersrange_observer(self,range,on_off):
        self.logger.debug("_numbersrange_observer called with: %s %s" % (range,("OFF", "ON")[on_off]))
    ...
    def get_score(self,timespend):
        ...
        # w = wrong entries
        # ts = total entries
        # ks = known entries (the number of entries shown on start)
        m,s = timespend.split(':')
