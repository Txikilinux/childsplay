...
class TextInput(SPWidgets.TextEntry):
    def __init__(self, pos, length=200, maxlen=12, message='', fsize = 28, border=True, \
                 ttf='arial', fgcol=BLACK, bgcol=None, password_mode=False):
        SPWidgets.TextEntry.__init__(self, pos=pos,length=length, maxlen=maxlen, message=message,\
                                     fsize=fsize, border=border, password_mode=password_mode, \
                                     ttf=ttf, fgcol=fgcol, bgcol=bgcol)
        self.logger = logging.getLogger("schoolsplay.SPVirtkeyboard.TextInput")
        self.fsize = fsize
        ...
        if self.message:
            for c in message:
                self.add(c)
        self._fl = ''# see KeyBoard._set_firstletter why this is used
    ...
    def add(self, c):
        if len(self.message) > self.maxlen-1:
            return
        self.message += c
        if self._fl and self.message[0] != self._fl:
            self.logger.debug("%s doesn't start with %s" % (self.message, self._fl))
            self._snd.play()
    ...
    def draw(self):
        ...
        if self.password_mode:
            self.messagesurf = utils.char2surf('*' * len(self.message) +'_', self.fsize, self.fgcol, ttf=TTFBOLD)
        else:
            self.messagesurf = utils.char2surf(self.message+'_', self.fsize, self.fgcol, ttf=TTFBOLD)
        self.image.blit(self.messagesurf, (0, 0))
        self.display_sprite()
...
class KeyBoard:
    ...
    def __init__(self, cbf, actives, keyboard_mode='qwerty', password_mode=False,\
                  ...
                  display_position=(0, 0), maxlen=22, display_length=0):
    ...
    def set_keymap(self, kbmap):
        ...
        self.TI = TextInput(pos=self.display_position, length=self.display_length, maxlen=self.maxlen, \
                            message='', fsize = 28, border=True, ttf=TTFBOLD, \
                            fgcol=BLACK, password_mode=self.password_mode)

    def _set_firstletter(self, c, snd):
        # special method, only used by various Argos acts.
        self.TI._set_firstletter(c, snd)
