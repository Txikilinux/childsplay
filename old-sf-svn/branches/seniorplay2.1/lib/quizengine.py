...
class ContentFeeder:
    ...
    def get_exercise(self, tp=None):
        ...
        """
        if self.UseWrongExercises:
            if self.wrongexercise:
                d, self.current_question_id = self.wrongexercise.pop()
...
#######################################################################        
        if getattr(self, 'idlist') and type(self.idlist) == types.DictType:
    ...
    def have_wrong_exercise(self):
        ...
        for k, v in self.served_dict.items():
            if not v[1]:
                ...
                self.wrongexercise.append((v[0], k))
        return self.wrongexercise
...
class Engine:
    ...
    def init_exercises(self,answers, maxtimes, difficulty, year=None):
        ...
        """
        self.logger.debug("init_exercises: answers %s, maxtimes %s, difficulty %s, year %s" % \
                          (answers, maxtimes, difficulty, year))
