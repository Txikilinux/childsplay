...
class Activity(quiz_general.Activity):
  ...
  """
    ...
    def _start_engine(self):
        try:
            self.quizengine = quiz_general.Engine(['history'], self.SPG, self.observer)
    ...
    def _prelevel_cbf(self, sprite, event, data):
        ...
        if kind != 'age':
            ...
            kb = KeyBoard(self._kb_cbf, self.actives, keyboard_mode='numbers',\
                      ...
                      display_position=(200, 250),maxlen=4, display_length=0)
            kb.show()
    ...
    def post_next_level(self):
        ...
        if self.level == 1:
            self.maxpoints = 2
            self.quizengine.init_exercises(2, self.num_questions, 1, self.year)
        else:
            self.maxpoints = 4
            self.quizengine.init_exercises(4, self.num_questions, 1, self.year) 
