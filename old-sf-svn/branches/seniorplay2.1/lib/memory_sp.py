...
class Activity:
  ...
  """
    ...
    def get_score(self,timespend):
        ...
        F1 = 2.0
        C = 3.0
        points = F / ( max( K / T, 1.0 ))
        time = min(F1 / ((S / T) ** C), F1)
        self.logger.debug("T %s K %s S %s points %s time %s" % (T, K, S, points, time))
        score = points + time
        return score
