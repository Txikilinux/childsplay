...
class Activity:
  ...
  """
    ...
    def get_score(self,timespend):
        ...
        self.logger.debug("N %s P %s S %s points %s time %s" % (N, P, S, points, time))
        result = points + time
        return result
                ...
                if y > 500 : break;
                    ...
                    if self.sequence == self.clicked_colors: 
                        ...
                        if self.throws == self.exercises:
                            ...
                            self.screen.blit(self.backgr_start,(0+15+self.xMargin,0+self.yTop+15))
                            pygame.display.update()
                    elif len(self.clicked_colors) == len(self.sequence) and \
                                            self.clicked_colors!=self.sequence:
    ...
    def yellow_clicked(self):
        if self.clickingEnabled==1: 
            ...
            if self.sequence[self.sequence_index] == CYELLOW:
                self.dbscore += 1
    ...
    def red_clicked(self):
        ...
        if self.clickingEnabled==1: 
            ...
            if self.sequence[self.sequence_index] == CRED:
                self.dbscore += 1    
        pygame.time.delay(600)
        pygame.event.clear()
        self.screen.blit(self.backgr_normal, (0+15+self.xMargin,0+self.yTop+15)) # this is needed here, as CP calls the start (and 
    ...
    def blue_clicked(self):
        ...
        if self.clickingEnabled==1: 
            ...
            if self.sequence[self.sequence_index] == CBLUE:
                self.dbscore += 1
        pygame.time.delay(600)
        pygame.event.clear()
        self.screen.blit(self.backgr_normal, (0+15+self.xMargin,0+self.yTop+15)) # this is needed here, as CP calls the start (and 
    ...
    def green_clicked(self):
        ...
        if self.clickingEnabled==1: 
            ...
            if self.sequence[self.sequence_index] == CGREEN:
                self.dbscore += 1
        pygame.time.delay(600)
        pygame.event.clear()
        self.screen.blit(self.backgr_normal, (0+15+self.xMargin,0+self.yTop+15)) # this is needed here, as CP calls the start (and 
