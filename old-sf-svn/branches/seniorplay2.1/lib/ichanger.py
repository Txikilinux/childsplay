...
class TextBox(SPSpriteUtils.SPSprite):
    def __init__(self, surf, pos):
        ...
        for txt in (ai.box1, ai.box2, ai.box3):
            s = SPWidgets.render_textrect(txt, 18, P_TTF, surf, WHITE, \
...
class Activity:
  ...
  """
    ...
    def __init__(self,SPGoodies):
        ...
        self.good_image = SPSpriteUtils.MySprite(utils.load_image(os.path.join(self.my_datadir,'good_%s.png' % self.lang)))
    ...
    def start(self):
        ...
        self.startbut = SPWidgets.SimpleButtonRound(_('Play'), self.but_pos[0], fsize=24)
    ...
    def get_score(self,timespend):
        ...
        self.logger.debug("M %s P %s S %s points %s time %s" % (M, P, S, points, time))
        result = points + time
        return result
