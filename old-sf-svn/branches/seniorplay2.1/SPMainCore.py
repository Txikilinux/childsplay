...
class MainCoreGui:
    def __init__(self, resolution=(800, 600), options=None, language=('en', False), fullscr=None):
        ...
        DEFAULTICONPATH = os.path.join(ACTIVITYDATADIR, 'SPData', 'themes', self.theme)
...
#        self.menubar.display_sprite()
    ...
    def start_next_level(self, store_db=None, nextlevel=None, levelup=False):
        ...
        # reset any clipping
        #if not nextlevel: breakpoint
        if not nextlevel:
            nextlevel = self.levelcount
        self.screen.set_clip()
...
#                pygame.event.set_blocked(MOUSEMOTION)
        ...
        while self.run_event_loop:
            ...
            for event in events:
                if event.type == KEYDOWN:
                    ...
                    elif event.key == K_F1 and self.logger.isEnabledFor(logging.DEBUG):
                        self.make_screenshot()
                    elif event.key == K_F2 and self.logger.isEnabledFor(logging.DEBUG):
                        self.mail_logfile()
    ...
    def make_screenshot(self):
        try:
            ...
            self.activity_info_dialog(text)
        else:
            mail_logfile(path)

    def mail_logfile(self, path=''):
        info = self._ask_input(_("Please give a short description or ticket number and hit [Enter] (max 50 characters)"))
        import Mail
        try:
            Mail.mail(info, path)
    ...
    def _ask_input(self, text):
        ...
        for line in textwrap.wrap(text,50):
            l = utils.char2surf(line, fsize=24, fcol=DARKGREEN,bold=True)
            self.screen.blit(l, (20,y))
            y += l.get_rect().h

        pygame.display.update()
        te = TextEntry((40,220), maxlen=50, length=700, ttf=TTFBOLD, fsize=20)
        info = te.ask()
        # this line only displays long enough to see it on slower connections
        # or if we have network troubles, the smtp client timesout in 5 seconds.
        line = _("Your email will be send, please wait.....")
        s = utils.char2surf(line, fsize=24, fcol=DARKGREEN,bold=True)
        pygame.display.update(self.screen.blit(s,(40,300)))
