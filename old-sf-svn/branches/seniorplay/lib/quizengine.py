...
from utils import load_image, load_sound, load_music, char2surf
from SPSpriteUtils import SPSprite, SPInit
from SPWidgets import TransPrevNextButton, ImgButton, TransImgButton

class ParserError(Exception):
    pass

# TODO: make a script that check the xml content prior to installing it.
#       WE don't check here if the xml is properly formatted

class ContentFeeder:
    """Extracts content from the SQL tables."""
    # TODO: Add support for knowledge quiz
    ...
    def _get_ids(self, tp, difficulty, year=None):
        ...
        if len(tp) == 1:
            if year:
                ...
                self.cursor.execute(
                        '''SELECT content_id FROM %s WHERE difficulty=?''' % tp[0],\
                        (difficulty, ))
...
#                    data TEXT, year TEXT, _group TEXT) 
        ...
        if type(self.idlist) == types.DictType:
        ...
        if id[0] in self.served:
            self.logger.debug("id %s already served, recurse." % id)
            # we must make sure we don't get into a endless loop.
            if self.recurse > 5:
                self.recurse = 0
                # This way we will get an new question id list, see above 
                del(self.idlist)
            else:
                self.recurse += 1
            self.get_exercise(tp)
        else:
            self.recurse = 0
...
class AudioPlayer:
    def __init__(self, audio, ad_audio):
    ...
    def play(self):
        ...
        if not self.wehaveaudio:
            ...
            return
        try:
            pygame.mixer.music.load(self.audiolist[0])
            pygame.mixer.music.play()
            for i in range(1, len(self.audiolist)):
                while pygame.mixer.music.get_busy():
                    pygame.time.wait(100)# prevent cpu hogging
                pygame.mixer.music.load(self.audiolist[i])
                pygame.mixer.music.play()
        except Exception, info:
            self.logger.exception("Unforeseen error in the playing of the soundstream")
...
class Engine:
    ...
    def __init__(self, quiz, spgoodies, observers, rchash=None):
        ...
        self.good_image = SPSprite(load_image(os.path.join(self.my_datadir,'good_%s.png' % self.lang)))
        self.good_image.moveto((300, 300))
        ...
        self.tryagain_image.moveto((270, 300))
        ...
        y = self.blit_pos[1]
        self.question_pos = (10, 10 + y)
        self.answer2_pos = [((10, 200 + y),iconA, 'a'), ((10, 365 + y), iconB, 'b')]
        self.answer4_pos = [((10, 190 + y), iconA, 'a'), ((10, 270 + y), iconB, 'b'),\
                             ((10, 350 + y), iconC, 'c'), ((10, 430 + y), iconD, 'd')]
        self.logo_pos = (600, 200 + y)
        self.snd_pos = (700, 410 + y)
        self.prev_pos = (600, 410 + y)
        ...
        self.good_sound=load_music(os.path.join(self.my_datadir,'good.ogg'))
        self.wrong_sound=load_music(os.path.join(self.my_datadir,'wrong.ogg'))
    ...
    def _clear_screen(self):
        ...
        # TODO: add proper logo, perhaps ask our parent ? HJ doesn't want a logo ?
        if self.logo_sprite:
    ...
    def _notify_obs(self):
        ...
        """
        self.logger.debug("_notify_obs called, maxresult = %s" % self.maxresult)
        # keep sprites for the 'previous screen'
        self.previous_list = self.answers_list[:]
        self.parent_observers(self.maxresult*10)
        if self.maxresult > -1:
            self.next_question()
    ...
    def init_exercises(self,answers, maxtimes, difficulty, year=None):
        ...
        self.difficulty = difficulty
        self.CF.set_difficulty(difficulty)
    ...
    def _next_question(self):
        ...
        if self.answers == 2:
            ...
            s_w = self._blit_on_me(s_w, ts, x=50)
            # and the red one
            s_w_r = self.answer2R_surf.convert_alpha()
            s_w_r = self._blit_on_me(s_w_r, ts, x=50)
            ...
            s_c = self._blit_on_me(s_c, ts,  x=50, y=4)
            # and the green one            
            s_c_g = self.answer4GR_surf.convert_alpha()
            s_c_g = self._blit_on_me(s_c_g, ts, x=50, y=4)
        ...
        if self.wehaveaudio:
            self.audioplayer = AudioPlayer(audiohash, self.ad_audiohash)
            self.audioplayer.play()
            self.snd_button.display_sprite()
        self.current_question_id = self.CF.served[-1]
