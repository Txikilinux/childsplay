...
class Activity:
  ...
  """
    ...
    def pre_level(self, level):
        ...
        self.logger.debug("pre_level called")
        self.level = level
        ...
        files.sort()
        lbl0 = SPWidgets.Label(_("Please, choose a set of cards."),\
                                (100, 120),fsize=24, transparent=True)
        lbl0.display_sprite()
    ...
    def start_exercise(self):
        ...
        try:
            ...
            files = glob.glob(os.path.join(imgdir,'*'))
        ...
        imagelist = random.sample(imagelist, self.num_of_cards/2)*2 # we must have doubles of course
    ...
    def scoreobs(self, i):
        ...
        if self.displaydscore <= 0:
            self.displaydscore = 0
        self.scoredisplay.set_score(self.displaydscore)
    ...
    def get_score(self,timespend):
        ...
        score =  8.0 / ( max( float(cardsturnt)/float(totalcards)/1.8, 1.0 ) )**c \
