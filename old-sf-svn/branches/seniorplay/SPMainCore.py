...
class MainCoreGui:
    def __init__(self, resolution=(800, 600), options=None, language=('en', False), fullscr=None):
        ...
        contentdir = os.path.join(ACTIVITYDATADIR,'CPData','Quizcontent', language[0][:2], 'content_*.xml')
        SPContentTables.check_tables(dbase, contentdir, language[0][:2])
...
#        self.menubar.display_sprite()
    ...
    def display_levelstart(self, level):
        ...
        if self.cmd_options.no_text or self.cmd_options.nocountdown:
            return
    ...
    def start_next_level(self, store_db=None, nextlevel=None, levelup=False):
        ...
        if self.levelcount == 0:
            self.levelcount = 1
        self.logger.debug("Calling activity next_level, level:%s" % self.levelcount)
    ...
    def core_chart_button_pressed(self):
        ...
        s = Graph(scorelist, self.levelcount, norm=norm, headertext=htext)
        dlg = Dialog(s, buttons=[_("Save image"),_("OK")], title=_('Results'))
        dlg.run()
        self.activity.refresh_sprites()
        result = dlg.get_result()
        if result == _('Save image'):
            ...
            d = Dialog(_("Your image is saved at:\n%s" % path), (200, 200),\
    ...
    def activity_level_end(self, store_db=None, nextlevel=None, levelup=False):
        ...
        self.screen.set_clip()
        self.clear_screen()
        if store_db and hasattr(self,'mapper'):
    ...
    def start_pre_level_loop(self):
        ...
        self.were_in_pre_level = False
        try:
            self.logger.debug("Start pre_level")
