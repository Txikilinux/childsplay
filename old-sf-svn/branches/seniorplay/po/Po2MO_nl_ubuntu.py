...
PONAME = 'seniorplay.po'
MONAME = 'seniorplay.mo'
PODIR = os.getcwd()
...
if len(sys.argv) == 2:
    ...
    files = ["%s/seniorplay_%s.po" % (PODIR, sys.argv[1])]
...
loclist = []
for name in files:
    loclist.append(os.path.basename(name).split('seniorplay_')[1][:5])
print loclist
...
for pofile in files:
    ...
    if os.path.exists(pofile) and 'latest' not in pofile:
        langlist = os.path.basename(pofile).split('seniorplay_')
        print langlist
        ...
        line = line[8:]
        trans = int(line.split('vertaalde')[0].strip())
