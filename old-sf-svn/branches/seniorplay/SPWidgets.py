...
def Init(theme):
    ...
    rc = os.path.join( GUITHEMESPATH, theme, 'theme.rc')
    if not os.path.exists(rc):
        module_logger.info('theme file %s does not exists, using default file' % rc)
        rc = os.path.join(DEFAULTGUITHEMESPATH,'theme.rc')
...
class TransImgButton(ImgButton):
    def __init__(self, path, hpath, pos=(0, 0), padding=4, name=''):
...
class Label(Widget):
    def __init__(self, txt, pos, fsize=18, padding=4, border=None, name='', ** kwargs):
    ...
    def settext(self, txt):
        s = utils.char2surf(txt, fsize=self.fsize,ttf=P_TTF, fcol=self.fgcol)
        r = s.get_rect()
        if self.minh:
            y = self.minh
            x = r.w
...
class TextView(Widget):
    def __init__(self, txt, pos, rect, fsize=12, padding=4, \
                 autofit=False, border=False, name='', **kwargs):
        ...
        fsize - Font size
        padding - space in pixels around the text
        fg_color - text color (R,G,B), if None the theme color will be used.
        bg_color - background color (R,G,B), if None the theme color will be used. 
...
class TextEntry(Widget):
    ...
    # Original license: GNU GPL version 2
    ...
    def __init__(self, pos, maxlen=12, message='', fsize = 28, border=True, *args):
        """Box which displays a text input. The height of the box is determined
        by the fontsize used and the length is determined by the maximum of 
        input characters.
        ...
        width = font.render("W" * maxlen, True, GREY).get_rect().w
        font.set_bold(True)
        question = font.render(message, True, THEME['textentry_fg_prompt_color'])
        self.ql = question.get_rect().w
        width += self.ql
        surf = pygame.Surface([width + 8, font.get_linesize() + 4])
...
class Dialog(Widget):
    ...
    dlg.erase_sprite()
    """
    def __init__(self, txt, pos=None, fsize=12, buttons=['OK'], title='', dialogwidth=300, name=''):
        ...
        for buttxt in self.buttons:
            b = Button(buttxt, (380, 250), self.fsize+8, padding=8, name=buttxt)
    ...
    def _setup(self):    
        ...
        title = utils.char2surf(self.title, 16, ttf=P_TTF)
        ...
        if type(self.txt) == types.ListType or type(self.txt) in types.StringTypes:
            if type(self.txt) == types.ListType:
                self.txt = '\n'.join(self.txt)
            tv = TextView(self.txt, (0, 0), r, bgcol=THEME['dialog_bg_color'] , autofit=True)
        ...
        #titlesurf.fill(THEME['dialog_bar_color'])
        titlesurf.fill(GRAY60)
...
def render_textrect(string, fsize, family, rect, text_color, background_color,\
                    justification=0, autofit=False, border=False, padding=8):
    ...
    if NoGtk:
        ...
        font = utils.pangofont.PangoFont(family=family, size=fsize)
