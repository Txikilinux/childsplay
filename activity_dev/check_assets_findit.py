...
if len(sys.argv) < 2:
    print "Usage: check_assets_findit.py <path to assets dir>"
    sys.exit(1)
...
c = db.cursor()
c.execute('''SELECT * FROM %s WHERE NOT ISNULL(file1OriginalName) AND \
                                    NOT ISNULL(file2OriginalName)AND\
...
missingcids = []
for cid in cids:
    files = glob.glob(os.path.join(p, "game_findit_file*_%s_width_470.jpg" % (cid)))
    if not files:
...
dest = os.path.join(p, 'checked', 'findit')
